...
1. **Генерация Диалогов**: Использует файлы `default.yaml`, `scene_generator.py` и `theme_generator.py` для создания динамических тем для сценариев.
2. **Синтез Речи**: Файлы в каталоге `services/voice/` отвечают за генерацию речи из текста.
3. **Unity Frontend**: `Main.cs` обеспечивает взаимодействие между клиентом на Unity и сервером на Python.
...
- `Main.cs`: Точка входа для приложения на C# (Unity). Этот файл обеспечивает взаимодействие между Unity и сервером на Python.
- `scene_generator.py`: Содержит базовые функциональности, используемые для генерации диалогов и других задач.
- `theme_generator.py`: Генерирует участников и темы для диалогов на основе заданных параметров из `default.yaml`.
...
Для добавления новых конфигураций создайте новый файл `.yaml` в каталоге `config/custom/` и обновите его в соответствии с вашими требованиями.
...
1. Импортируйте файл `Main.cs` в ваш проект Unity.
2. Убедитесь, что все настройки и зависимости настроены правильно.
3. Добавьте URL сервера Python в настройках Unity.
4. Запустите сцену, в которой используется `Main.cs`.
...
Скрипт начинает свою работу с загрузки конфигурационного файла `default.yaml`. В этом файле содержатся настройки, которые определяют структуру и содержание будущих диалогов. Скрипт создаёт список возможных тем для диалога с участниками и сохраняет их в файле `scenarios/{config-name}/themes.txt`. 
...
После получения диалога, скрипт разделяет его на отдельные строки и отправляет их на озвучивание. Затем длительность каждого аудиофрагмента измеряется, и на основе этой информации создается текстовый файл сценария. Все аудиофрагменты объединяются в один файл и сохраняются вместе с текстовым сценарием в каталоге `scenarios`.

### Отдача контента

При запуске генерации сценариев также запускается веб-сервер с следующими функциями:

- `GET /scenarios`: Получение списка доступных сценариев.
- `GET /audio/<int:scenario_number>`: Получение аудиофайла для указанного сценария.
- `GET /script/<int:scenario_number>`: Получение текстового файла сценария.
- `DELETE /delete/<int:scenario_number>`: Удаление сценария.

### Скрипт для Unity

Скрипт в Unity обращается к веб-серверу для получения списка доступных сценариев и выбирает из него первый. Далее запрашивает соответствующий текстовый и аудио файл. После этого начинается воспроизведение аудио, и камера в Unity переключается на модель персонажа, устанавливается текст в текстовый объект, в соответствии с текстовым сценарием.
